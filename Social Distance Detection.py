...
def draw_distance(frame, idf, boxes, WIDTH, HEIGHT, high_risk, low_risk, 
  ...
  for i in idf:
    ...
    cv2.putText(frame, "Riesgo Alto : {}".format(high_risk),(25, HEIGHT - 65),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 150), 2)
    cv2.putText(frame, "Riesgo Bajo : {}".format(low_risk), (25, HEIGHT - 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 120, 255), 2)
    cv2.putText(frame, "Seguro      : {}".format(safe), (25, HEIGHT - 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6,  (255, 255, 255), 2)
...
def detection_video_file(video_path, yolo_weights, yolo_cfg, coco_names, 
  ...
  while True:
    ...
    if writer is None:
        ...
        writer = cv2.VideoWriter("video_salida_3.avi", fourcc, 30.0, (frame.shape[1], frame.shape[0]))
    writer.write(frame)
